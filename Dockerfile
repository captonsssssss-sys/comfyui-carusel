...
RUN if [ -f "${COMFYUI_PATH}/custom_nodes/ComfyUI-llama-cpp_vlm/requirements.txt" ]; then python3 -m pip install --no-cache-dir -r "${COMFYUI_PATH}/custom_nodes/ComfyUI-llama-cpp_vlm/requirements.txt"; fi
...
RUN python3 -c 'from pathlib import Path; import ast; p=Path("/default-comfyui-bundle/ComfyUI/custom_nodes/ComfyUI-llama-cpp_vlm/nodes.py"); s=p.read_text(encoding="utf-8"); ast.parse(s); required=["llama_cpp_model_loader","llama_cpp_parameters","llama_cpp_instruct_adv"]; missing=[x for x in required if x not in s]; assert not missing, "Missing nodes: " + ", ".join(missing); print("Static nodes.py check: OK")'
...
RUN python3 -c 'import json; from pathlib import Path; p=Path("/tmp/CARUSEL.json"); data=json.loads(p.read_text(encoding="utf-8")); assert isinstance(data,dict), "CARUSEL.json root must be an object"; nodes=data.get("nodes",[]); print("Workflow JSON: OK"); print("Workflow nodes:",len(nodes)); types={n.get("type") for n in nodes if isinstance(n,dict)}; required=["llama_cpp_model_loader","llama_cpp_parameters","llama_cpp_instruct_adv"]; missing=[x for x in required if x not in types]; assert not missing, "Required workflow nodes missing: " + ", ".join(missing); print("Required Llama nodes: OK")'
