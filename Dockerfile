...
RUN PY_MM=$(python3 -c "import sys; print(str(sys.version_info.major) + str(sys.version_info.minor)") && if [ "$PY_MM" = "310" ]; then LLAMA_WHEEL="llama_cpp_python-0.3.49+cu131-cp310-cp310-linux_x86_64.whl"; elif [ "$PY_MM" = "311" ]; then LLAMA_WHEEL="llama_cpp_python-0.3.49+cu131-cp311-cp311-linux_x86_64.whl"; elif [ "$PY_MM" = "312" ]; then LLAMA_WHEEL="llama_cpp_python-0.3.49+cu131-cp312-cp312-linux_x86_64.whl"; elif [ "$PY_MM" = "313" ]; then LLAMA_WHEEL="llama_cpp_python-0.3.49+cu131-cp313-cp313-linux_x86_64.whl"; elif [ "$PY_MM" = "314" ]; then LLAMA_WHEEL="llama_cpp_python-0.3.49+cu131-cp314-cp314-linux_x86_64.whl"; else echo "Unsupported Python version: $PY_MM"; exit 1; fi && echo "$LLAMA_WHEEL" > /tmp/llama_wheel

RUN LLAMA_WHEEL=$(cat /tmp/llama_wheel) && wget -q --show-progress -O "/tmp/$LLAMA_WHEEL" "https://github.com/JamePeng/llama-cpp-python/releases/download/v0.3.49-cu131-linux-20260831/$LLAMA_WHEEL"

RUN python3 -m pip uninstall -y llama-cpp-python || true

RUN python3 -m pip install --no-cache-dir --force-reinstall /tmp/llama_cpp_python-*.whl

RUN rm -f /tmp/llama_cpp_python-*.whl /tmp/llama_wheel
...
RUN python3 -c "import llama_cpp; from llama_cpp import Llama; print('llama_cpp version:', getattr(llama_cpp, '**version**', 'unknown')); print('Llama: OK')"
